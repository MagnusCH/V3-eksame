...
$time = time();
$forfatter = $_SESSION['username'];
// php funktion, angiver time, hvor mange sek der er gået fra 1970 og til nu, som vi kan lave om til en date, vi har ikke skrevet det i formlen så man ikke selv skal skrive den ned, når time gør den selv.

// laver en forbindelse over til connect.php
require_once "connect.php";
// laver en variaben $statement, og indsætter vores $DHB
// insert into : smider vores til værdier ind i article (imgSrc ..) tager de værdier ind i article
// value(?..) er placeholders, så folk ikke kan skrive direkt ind i databasen
$statement = $DBH->prepare("INSERT INTO articles (imgSrc, imgAlt, heading,  time, articleText, forfatter) values(?, ?, ?, ?, ?, ?)");
...
$statement->bindParam(6, $forfatter);
$statement->execute();
